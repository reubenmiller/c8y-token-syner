...
wget -O - example/install.sh | sh -s -- --enrol 'https://%s/service/c8y-token-syner/register/%s' --code '%s'
	`, tenant.DomainName, externalID, code)

	installScriptDocker := fmt.Sprintf(`
	./tools/trial-bootstrap-docker --enrol 'https://%s/service/c8y-token-syner/register/%s' --code '%s'
	`, tenant.DomainName, externalID, code)
